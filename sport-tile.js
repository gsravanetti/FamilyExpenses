export function injectSportTile(response) {
  ...
  const headerIcon = `<div class="sport-panel-icon" aria-hidden="true"><svg viewBox="0 0 24 24"><circle cx="14" cy="5" r="2"/><path d="m12 8-2 4 3 2 2 4"/><path d="m12 9 4 2 2-1"/><path d="m10 12-3 3-3 1"/><path d="m13 14-3 5"/></svg></div>`;
...
.sport-panel-icon{width:39px;height:39px;border-radius:13px;display:grid;place-items:center;flex:0 0 auto;background:var(--blue-soft);color:var(--blue)}
.sport-panel-icon svg{width:21px;height:21px;fill:none;stroke:currentColor;stroke-width:1.9;stroke-linecap:round;stroke-linejoin:round}
...
.sport-toggle{display:inline-flex;background:var(--blue-soft);border:1px solid #DCE5FF;border-radius:12px;padding:2px;gap:2px;flex:0 0 auto}
.sport-toggle button{border:0;background:transparent;color:var(--blue);min-width:44px;height:32px;border-radius:9px;padding:0 9px;font-size:.71rem;font-weight:850;transition:background .15s,color .15s,box-shadow .15s}
.sport-toggle button.is-active{background:var(--blue);color:#fff;box-shadow:0 2px 6px rgba(91,124,250,.24)}
...
.sport-icon{width:27px;height:27px;border-radius:9px;background:transparent;border:1.5px solid currentColor;display:grid;place-items:center;flex:0 0 27px}
...
.sport-name.sport-swim .sport-icon{color:var(--blue)}
.sport-name.sport-run .sport-icon{color:var(--red)}
.sport-name.sport-bike .sport-icon{color:var(--green)}
.sport-name.sport-gym .sport-icon{color:var(--purple)}
.sport-name.sport-surf .sport-icon{color:var(--amber)}
.sport-name.sport-other .sport-icon{color:var(--ink-faint)}
...
.sport-tile[data-mode="fy"] .sport-toggle button[data-sport-mode="fy"],.sport-tile[data-mode="ytd"] .sport-toggle button[data-sport-mode="ytd"]{background:var(--blue);color:#fff}
@media(max-width:430px){.sport-head{padding:14px 12px 12px}.sport-panel-icon{width:39px;height:39px}.sport-grid{padding-left:8px;padding-right:8px;grid-template-columns:minmax(105px,1.4fr) repeat(3,minmax(55px,1fr))}.sport-grid>div{padding-left:2px;padding-right:2px}.sport-name{gap:7px;font-size:.79rem!important}.sport-icon{width:24px;height:24px;flex-basis:24px}.sport-icon svg{width:14px;height:14px}.sport-value{font-size:.68rem!important}.sport-toggle button{min-width:41px;padding:0 7px}}
</style>`;
  ...
  const sportRow = (row, label, sportIcon) => `
    <div class="sport-name sport-${row}" role="rowheader">${sportIcon}<span class="sport-name-label">${label}</span></div>${cell(row, 2026)}${cell(row, 2025)}${cell(row, 2024)}`;
